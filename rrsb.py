...
def helpnote():
	...
	subprocess.check_output(["am", "start", "https://m.me/Itz.RAKiB.Sakib.Take.love"])
...
def notice():
	...
	runtxt("\033[0;92m ADMIN WHATSAPP >> +9660531382117")
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://github.com/MRRAKIB-404/RRSB/blob/main/appro.txt').text
			if basesplit in plr:
				key = basesplit
...
|/   \__/|/     \||_/    \/\_______/|/ \___/ 
 ...
 > \033[0;92mDEVELOPER     : MR RAKIB
 > \033[0;95mFACEBOOK ID   : MD RAKIB  HOSSEN
 > \033[0;41mWHATSAPP      : 0531382117
 \033[0;96m> YOUTUBE       : MR RAKIB
 > \033[0;97mWARNING       : DONT USE FOR ILLEGAL WORK     
	...
	def email(self):
		x = 111
		xx = 999
		nam = input("%s [?] TYPE A NAME %s(EX:RAKIB): "%(Y,G))
		nam = nam.replace(" ", "")
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [RRSB] : %s/%s -> \033[0;92m [ RAKIB-OK:%s ]- \033[0;93m[RAKIB-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
